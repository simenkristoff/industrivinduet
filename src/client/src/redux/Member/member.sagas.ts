...
import { setMembers, fetchMembersStart, setMember } from './member.actions';
...
export function* addMember({ payload }: any) {
  try {
    yield MemberService.create({
      ...payload,
    });
    yield put(fetchMembersStart());
...
export function* fetchMember(payload: string) {
  try {
    const member = yield MemberService.find(payload);
    yield put(setMember(member));
  } catch (err) {
    console.log(err);
  }
}
...
export function* deleteMember(payload: string) {
  try {
